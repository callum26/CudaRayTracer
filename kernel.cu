    ...
    __device__ bool rayIntersect(Ray ray, Object object, float &distance){
        ...
        Vec3 sphereToOrigin = ray.origin.sub(object.pos);
    ...
    {
        // D = kd * (Lm . N)* im,d
        Vec3 hitPoint = ray.origin.add(ray.rayDir.scale(distanceToObject));
    ...
    __device__ Vec3 calculateSpecular(Ray ray, Light light, float distanceToObject, Object object)
    {
        ...
        Vec3 camToHit = ray.origin.sub(hitPoint);
